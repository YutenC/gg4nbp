    ...
    const shoppingContent = Vue.createApp({
        data() {
            return {
                ...
                // 配送方式
                deliver: 'takuhai',
                address: { county: '桃園市', area: '', address: '' },
                fullAddressList: [],
                countySelet: [],
                // 付款方式
                payment: 'credit',
        ...
        computed: {
            ...
            areaList() {
                let fitNeed = this.fullAddressList.filter(item => item.city_name === this.address.county);
                return Array.from(fitNeed, item => item.name)
            },
            deliverCal() {
        ...
        created() {
            this.renewList();
            this.getBonusStock();
            axios.get('https://demeter.5fpro.com/tw/zipcodes.json')
                .then(res => {
                    this.fullAddressList = res.data;
                    this.countySelet = new Set();
                    this.fullAddressList.forEach(item => this.countySelet.add(item.city_name));
                })
                .catch(err => console.log(err))
        }
